...
export default function ConferenceBanner() {
  ...
  return (
    <section className="w-full flex justify-center px-4 py-10 font-sans" ref={ref}>
      <div className="relative w-full max-w-5xl min-h-[400px] overflow-hidden rounded-[160px] bg-[#ddeef8]">
        {/* Right - skyline / Merlion only */}
        <div
        ...
        {/* Left - content panel with curved right edge */}
        <motion.div
        ...
        >
          ...
          <motion.div style={{ marginBottom: "8px" }} {...fadeUp(0.26)}>
            {[
              "Founder & CEO, Aurify Technology",
              "12+ years · Precious metals · Digital transformation",
            ].map((line, i) => (
              <p
                key={i}
                style={{ fontSize: "12px", color: "#1a1a1a", margin: "2px 0", lineHeight: 1.5 }}
              >
                {line}
              </p>
            ))}
          ...
          <motion.div {...fadeUp(0.4)}>
            <p style={{ fontSize: "11px", color: "#1a1a1a", marginBottom: "4px" }}>
              Fix a meeting or reach out directly
            </p>
        ...
        {/* CEO name - left of person at the seam */}
        <motion.div
          className="absolute bottom-11 z-30"
          style={{ left: "36%" }}
          {...fadeUp(0.48)}
        >
          <p style={{ fontSize: "13px", fontWeight: 700, color: "#0d1f33", whiteSpace: "nowrap" }}>
            Muneeb Cholayil
          </p>
          <p style={{ fontSize: "11px", color: "#3d5a73" }}>CEO &amp; Founder</p>
        </motion.div>

        {/* Muneeb cutout - overlaps both sections */}
        <motion.img
